...
class dataRoot
{
    ...
    public function updateByID($table,$updateKey, $updateValue, $idKey, $idValue){
        $updateKey = addslashes($updateKey);
        $updateValue = addslashes($updateValue);
        $idKey = addslashes($idKey);
        $idValue = addslashes($idValue);
        return $this->conn->query("UPDATE $table set `$updateKey` = '$updateValue' WHERE `$idKey`='$idValue'");
//        return true;
    }
